...
## Objectives
Use `AlphaSimR` to simulate founders, then generate a pedigree of four generations, either through random mating or through selection.  
...
## The "true" relationship matrix is that of QTL
We want to estimate a relationship matrix as close to this as possible

## Create a SNP relationship matrix
The SNP relationship matrix is another realization. Assuming SNPs are in LD with the QTL, it should be more strongly correlated to the causal matrix than the pedigree-based relationship matrix.

## Create a pedigree relationship matrix
This is the expectation of the QTL relationship matrix. The QTL relationship matrix is one realization.
...
heatmap(aMat[pickVisu, pickVisu], symm=T, Rowv=NA, Colv=NA, revC=T)
heatmap(snpRelMat[pickVisu, pickVisu], symm=T, Rowv=NA, Colv=NA, revC=T)

# Off diagonal element plots
plot(offDiag(aMat), offDiag(qtlRelMat), pch=16, main="QTL against Pedigree", xlab="Pedigree pairwise", ylab="QTL pairwise")
plot(offDiag(aMat), offDiag(snpRelMat), pch=16, main="SNP against Pedigree", xlab="Pedigree pairwise", ylab="SNP pairwise")
plot(offDiag(snpRelMat), offDiag(qtlRelMat), pch=16, main="QTL against SNP", xlab="SNP pairwise", ylab="QTL pairwise")

# Diagonal element plots
plot(diag(aMat), diag(qtlRelMat), pch=16, main="QTL against Pedigree", xlab="Pedigree 1 + F", ylab="QTL 1 + F")
plot(diag(aMat), diag(snpRelMat), pch=16, main="SNP against Pedigree", xlab="Pedigree 1 + F", ylab="SNP 1 + F")
plot(diag(snpRelMat), diag(qtlRelMat), pch=16, main="QTL against SNP", xlab="SNP 1 + F", ylab="QTL 1 + F")
```
